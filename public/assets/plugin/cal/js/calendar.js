...
export const Calendar = (id) => ({ 
    ...
    render(y, m) {
        ...
        const isLowerDate = (d1, d2) => d1.format('YYYY-MM-DD') < d2.format('YYYY-MM-DD');
        ...
        for(let i=0; i<6; i++){ //loop thru 35 boxes on the calendar month
            ...
            <ol class="days list-unstyled" week="${d.week()}">
                <li class="${d.add(1,'d'),this.m != d.month()?' outside':'inside'}${isSameDate(d,now)?' today':''} ${isLowerDate(d,now)?' disabled':'1'}"><a class="date" onclick="showSelectedSlots('${d.year()}-${d.month() + 1}-${d.format('D')}',this)">${d.format('D')}</a><div class="events"></div></li>
                <li class="${d.add(1,'d'),this.m != d.month()?' outside':'inside'}${isSameDate(d,now)?' today':''} ${isLowerDate(d,now)?' disabled':'1'}"><a class="date" onclick="showSelectedSlots('${d.year()}-${d.month() + 1}-${d.format('D')}',this)">${d.format('D')}</a><div class="events"></div></li>
                <li class="${d.add(1,'d'),this.m != d.month()?' outside':'inside'}${isSameDate(d,now)?' today':''} ${isLowerDate(d,now)?' disabled':'1'}"><a class="date" onclick="showSelectedSlots('${d.year()}-${d.month() + 1}-${d.format('D')}',this)">${d.format('D')}</a><div class="events"></div></li>
                <li class="${d.add(1,'d'),this.m != d.month()?' outside':'inside'}${isSameDate(d,now)?' today':''} ${isLowerDate(d,now)?' disabled':'1'}"><a class="date" onclick="showSelectedSlots('${d.year()}-${d.month() + 1}-${d.format('D')}',this)">${d.format('D')}</a><div class="events"></div></li>
                <li class="${d.add(1,'d'),this.m != d.month()?' outside':'inside'}${isSameDate(d,now)?' today':''} ${isLowerDate(d,now)?' disabled':'1'}"><a class="date" onclick="showSelectedSlots('${d.year()}-${d.month() + 1}-${d.format('D')}',this)">${d.format('D')}</a><div class="events"></div></li>
